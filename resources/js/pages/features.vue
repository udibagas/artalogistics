<template>
  <div>
    <div class="mb-4 d-flex">
      <h4 class="flex-grow-1">FEATURES</h4>
      <el-button
        type="primary"
        size="small"
        icon="el-icon-plus"
        @click="() => { formModel = {}; showForm = true; imageUrl = ''; }"
      >ADD FEATURE</el-button>
    </div>
...
export default {
  ...
  methods: {
    getData() {
      axios.get("feature").then(r => (this.features = r.data));
    },
    edit(data) {
    ...
    update() {
      axios
        .put(`feature/${this.formModel.id}`, this.formModel)
        .then(r => {
          this.$message({
            message: r.data.message,
            type: "success",
